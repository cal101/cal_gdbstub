...
uart0_rx_intr_handler(void *para)
{
    ...
    uint8 RcvChar;
#ifdef BRK_ENABLED
    if (UART_BRK_DET_INT_ST == (READ_PERI_REG(UART_INT_ST(UART0)) & UART_BRK_DET_INT_ST)) {
	WRITE_PERI_REG(UART_INT_CLR(UART0), UART_BRK_DET_INT_CLR);
	//breaks++;
	// TODO: allows only one break to avoid loops.
	if (!breakdisabled) {
	    breakdisabled = 1;
	    // TODO: check need
	    //ets_intr_lock();
